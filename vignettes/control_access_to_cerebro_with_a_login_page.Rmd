...
the Viewer decrypts the credentials table once into trusted process memory and
does not use SQLite as a writable runtime store. Update accounts or passwords
in the private source database, then rebuild and redeploy the App.

This administrator-managed mode intentionally has no in-App password changes,
SQLite login history, or database-backed failed-attempt counters and locks.
Configure source-aware rate limiting at the HTTPS reverse proxy before exposing
the Viewer to untrusted networks.
...
**Run it on:** the HTTPS reverse proxy in front of the Viewer.

**What happens:** the proxy limits repeated login requests from the same source
before they reach the Viewer.

**Verify:** use a disposable client to exceed the configured rate and confirm
that the proxy rejects or delays subsequent requests.

`shinymanager.pwd_failure_limit` does not apply in this read-only, in-memory
credentials mode. Reverse-proxy rate limiting is therefore required rather than
an optional second layer. Use a source-aware policy to avoid turning a shared
account lock into a denial-of-service mechanism.
...
- Add monitoring and source-aware reverse-proxy rate limiting.
