...
There is no Seurat object here (no cells), so we build the `Cerebro` object **by hand**.
...
```{r, eval = FALSE}
x <- readCerebro("demo_hla_tcr_bulk_toy.crb")
...
With your own data you build the *same two objects* from real files, then run Option B's assembly block unchanged — there is no Seurat object (bulk has no cells), so you set the `Cerebro` slots by hand exactly as shown above.
